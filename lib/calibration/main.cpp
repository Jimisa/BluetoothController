...
#define joystick_X 13    //joystick  X-as
#define joystick_Y 12    //joystick  Y-as
#define button_2 2       // button on d2


//****************define the min, max and center you want for your joystick here*******//
long range_X_min = -2047;           //lowest X value
long range_X_max = 2047;          //highest X value
...
float range_Y_min = -2047;             //lowest Y value
float range_Y_max = 2047;          //highest Y value
...
long joy_Y(long Y) {
  ...
  if (Y <= range_Y_center) {
    ...
    Y = mapfloat(Y, 0, 2048 + Y_CENTER, range_Y_center , range_Y_max);
    return Y;
  }
}
